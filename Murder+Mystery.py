...
def build_frequency_table(corpus):
    ...
    for element in corpus:
        if element in frequency_table.keys():
            frequency_table[element] = frequency_table[element] + 1
        else:
            frequency_table[element] = 1
    corpus = list(frequency_table.keys())
    corpus.sort() 
...
def ngram_creator(text_list):
    pairs = []
    pairs1 = []
    for txt in range(len(text_list) -1):
        pairs = text_list[txt] + ' ' + text_list[txt+1]
        pairs1.append(pairs)
    return pairs1
...
def frequency_comparison(table1, table2):
    ...
    for tab in table1.keys():
        if tab == table2.keys():
            if table1[key] > table2[key]:
                table2[key] += mutual_appearances
                table1[key] += appearances
            else:
                table1[key] += appearances
    for tab in table2.keys():
        if tab not in table1.keys():
            table2[key] += appearances
            freq_comp_score = mutual_apparances / appearances
    return freq_comp_score
...
def find_text_similarity(TextSample)
