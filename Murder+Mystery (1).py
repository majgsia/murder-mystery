...
def get_average_sentence_length(text):
    ...
    for sentence in text2_split:
        sentences_in_text.append(sentence)
        for word in sentences_in_text:
            sentence_lengths.append(word)
    sentences = len(sentences_in_text)
    words = len(sentence_lengths)
    average_length = (words) / (sentences)
    return str(average_length)

    sentences, words, average_length = get_average_sentence_length(murder_note)
    print("There are " + str(sentences) + " sentences and " + str(words) + " words in this text. The average sentence length is " + str(average_length).format(sentences=sentences,words=words,average_length = average_length))
...
class TextSample:
    ...
    def __repr__(self):
        return "{author} has an average sentence length of {average_sentence_length} ".format(author=self.author,average_sentence_length=self.average_sentence_length)
    ...
    for word1,frequency1 in table1.items():
        if word1 in table2:
            if table1[word1] > table2[word1]:
                appearances += table1[word1]
                mutual_appearances += table2[word1]
            else:
                appearances += table2[word1]
                mutual_appearances += table1[word1]
        else:
            appearances += table1[word1]

    for word2,frequency2 in table2.items():
        if word2 not in table1:
            appearances += table2[word2]
...
def percent_difference(ValOne, ValTwo):
    perc_diff = abs(float(ValOne) - float(ValTwo)) / ((float(ValOne) + float(ValTwo)) / 2)
    return perc_diff
...
def find_text_similarity(TextSampleOne, TextSampleTwo):
    sentence_length_difference = percent_difference(TextSampleOne.average_sentence_length, TextSampleTwo.average_sentence_length)
    sentence_length_similarity = abs(1 - sentence_length_difference)
    word_count_similarity = frequency_comparison(TextSampleOne.word_count_frequency, TextSampleTwo.word_count_frequency)
    ngram_similarity = frequency_comparison(TextSampleOne.ngram_frequency, TextSampleTwo.ngram_frequency)
...
print(lily_sample.author + "'s similarity score to the murder letter is " + str(find_text_similarity(lily_sample, murderer_sample)))
print(myrtle_sample.author + "'s similarity score to the murder letter is " + str(find_text_similarity(myrtle_sample,murderer_sample)))
print(gregg_sample.author + "'s similarity score to the murder letter is " + str(find_text_similarity(gregg_sample, murderer_sample)))
...
print("It was Lily Trebuchet who killed Jay Stacksby")
